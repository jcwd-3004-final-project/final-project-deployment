...
const getAvatarUrl = (avatar?: string): string => {
  ...
  return avatar.startsWith("http")
    ? avatar
    : `https://d29jci2p0msjlf.cloudfront.net/${avatar.replace(/\\/g, "/")}`;
};
  ...
  const handleEditAddressSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `https://d29jci2p0msjlf.cloudfront.net/v1/api/user/addresses/${editingAddressId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    try {
      ...
      if (isDefault) {
        ...
        if (remainingAddresses.length > 0) {
          ...
          await fetch(
            `https://d29jci2p0msjlf.cloudfront.net/v1/api/user/addresses/${newDefaultAddress.address_id}`,
            {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
