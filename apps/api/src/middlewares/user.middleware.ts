...
export class AuthenticateJwtMiddleware {
  public authenticateJwt(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!secret) {
        console.error(
          "JWT_ACCESS_TOKEN_SECRET is not set in environment variables."
        );
        res
          .status(500)
          .json({
  ...
  public authorizeRole(expectedRole: string | string[]) {
    return (req: Request, res: Response, next: NextFunction) => {
      const user = (req as any).user;

      // Jika expectedRole berupa array, periksa apakah user.role termasuk dalam array tersebut
      if (Array.isArray(expectedRole)) {
        ...
        // Jika expectedRole hanya string tunggal
